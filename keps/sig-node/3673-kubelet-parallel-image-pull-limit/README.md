...
nitty-gritty.
-->
* A new integer field `maxParallelImagePulls` will be added to Kubelet Configuration. `maxParallelImagePuls` is the maximum number of inflight image pulls.
* The default value of `maxParallelImagePulls` is 0, meaning no limit on parallel image pulls, which is same as the existing behavior.
* When `serialize-image-pulls` is true, the only valid value of `maxParallelImagePuls` is 0, and kubelet will throw a fatal error if it is not 0.
...
#### Story 1
A kubernetes user wants to enable parallel image pulls to increase workload startup latency, but they don't want too many images to be pulled at the same time, as that might burden the disk IO and has a negative impact on disk performance. By setting a proper `maxParallelImagePulls`, they will have a better control over the parallelism and therefore avoid overwhelming the disk.
...
* The implementation will be similar to the existing `serialImagePuller`. More specifically:
  * `maxParallelImagePulls` will be passed to [parallelImagePuller](https://github.com/kubernetes/kubernetes/blob/054ceab58d803df0c4910fa722138f0ed2d94701/pkg/kubelet/images/puller.go#L41), and `parallelImagePuller` will create a channel of the size of `maxParallelImagePulls`.
  * `parallelImagePuller.pullImage` will simply try to send the pull request to the channel, and will be blocked if the channel is full.
  * A go routine will keep processing the requests in the channel.
...
A new node_e2e test will be added to make sure that when maxParallelImagePulls is reached, all further image pulls will be blocked.
...
The feature will be enabled when `maxParallelImagePulls` is set to a non-zero value on kubelet, can be rolled back by setting `maxParallelImagePulls` to 0.
...
- [X] Other
  - Describe the mechanism: The feature will be enabled when `maxParallelImagePulls` is set to a non-zero value on kubelet, can be disabled by setting `maxParallelImagePulls` to 0 and restarting kubelet.
...
This will be added to the node_e2e test.
